...
public class Program4 {
    ...
    private static final String PROCESS_NAME = "P4";
    ...
    private static void executeLocalInstructions() {
        try {
            ...
            sendUniqueMessage("m1_to_P1", PEER_ADDRESSES[0]);
            // sendUniqueMessage("m3_to_P3", PEER_ADDRESSES[2]);
            // sendUniqueMessage("m3_to_P3", PEER_ADDRESSES[2]);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    ...
    private static void sendMessage(String content, String address) {
        ...
        while (retries > 0) {
            try (Socket socket = new Socket(parts[0], Integer.parseInt(parts[1]));
                ...
                out.writeObject(new Message("P4", content, vectorClock.getClock()));
