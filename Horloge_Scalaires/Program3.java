...
public class Program3 {
    ...
    private static void addEventWithSleep(String eventName) throws InterruptedException {
        incrementClock();
        MessageTrackerClient.addEvent("P3", eventName, scalarClock.get());
        System.out.println("P3 event: " + eventName + " at clock: " + scalarClock.get());
        Thread.sleep(500);
    }
    ...
    private static void sendMessageWithConfirmation(String content, String address) throws InterruptedException {
        ...
        System.out.println("P3 sending to " + address + ": " + content + " at clock: " + sendClock);
    ...
    private static void handleClient(Socket socket) {
        try (ObjectInputStream in = new ObjectInputStream(socket.getInputStream())) {
            ...
            MessageTrackerClient.addEvent("P3", eventDesc, scalarClock.get());
            MessageTrackerClient.addMessage(msg.getFromProcess(), "P3", msg.getContent(), 
                                         msg.getTimestamp(), scalarClock.get());
            
            System.out.println("P3 processed receive at clock: " + scalarClock.get());
    ...
    private static void sendMessage(String content, String address) {
        ...
        while (retries > 0) {
            try (Socket socket = new Socket(parts[0], Integer.parseInt(parts[1]));
                ...
                out.writeObject(new Message("P3", content, scalarClock.get()));
