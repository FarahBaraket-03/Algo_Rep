...
public class Program4 {
    private static final AtomicReference<MatrixClock> matrixClock = 
        new AtomicReference<>(new MatrixClock("P4", 4));
    ...
    private static void executeLocalInstructions() {
        try {
            ...
            sendMessageWithConfirmation("m3_to_P1", PEER_ADDRESSES[0]);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    private static void addEventWithSleep(String eventName) throws InterruptedException {
        incrementClock();
        MessageTrackerClient.addEvent("P4", eventName, matrixClock.get().getClock());
        System.out.println("P4 event: " + eventName + " at clock: " + matrixClock.get());
        Thread.sleep(500);
    }
    ...
    private static void sendMessageWithConfirmation(String content, String address) throws InterruptedException {
        ...
        System.out.println("P4 sending to " + address + ": " + content + " at clock: " + matrixClock.get());
    ...
    private static void handleClient(Socket socket) {
        try (ObjectInputStream in = new ObjectInputStream(socket.getInputStream())) {
            ...
            MessageTrackerClient.addEvent("P4", eventDesc, matrixClock.get().getClock());
            MessageTrackerClient.addMessage(msg.getFromProcess(), "P4", msg.getContent(), 
                                         msg.getClock().getClock(), matrixClock.get().getClock());
            
            System.out.println("P4 processed receive at clock: " + matrixClock.get());
